...
export class TrainingInstituteFormComponent implements OnInit {
  ...
  initForm() {
    this.instituteForm = this.fb.group({
      name: [
        this.trainingInstitute ? this.trainingInstitute.name : "",
        Validators.required
      ],
      ...
      contact_person: this.trainingInstitute
        ? this.trainingInstitute.contact_person
        : "",
      contact_no: [
        this.trainingInstitute ? this.trainingInstitute.contact_no : "",
        [Validators.pattern("[0-9]{10}")]
      ],
      contact_no2: [
        this.trainingInstitute ? this.trainingInstitute.contact_no2 : "",
        [Validators.pattern("[0-9]{10}")]
      ]
    });
  }
  ...
  onSubmit() {
    ...
    if (this.trainingInstitute && this.trainingInstitute.id) {
      this.instituteService
        .editTrainingInstitute(
          this.trainingInstitute.id,
          this.instituteForm.value
        )
        .subscribe(
          (newValue: TrainingInstitute) => {
      ...
      this.instituteService
        .addTrainingInstitute(this.instituteForm.value)
        .subscribe(
