...
export class FeedbackFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.feedback = this.training.training_feedbacks.find(
      feed => feed.emp_code === this.auth.currentUser.emp_code
    );
    ...
    if (this.training.training_type === IN_HOUSE_TRAINING) {
      this.training.training_topics.forEach(topic => {
        this.topicRatings.push(
          this.fb.group({
            rating: [
              topic.rating ? topic.rating.toString() : "",
              Validators.required
            ],
  ...
  initForm() {
    this.feedbackForm = this.fb.group({
      emp_code: this.auth.currentUser.emp_code,
      ta_da_incurred: [
        this.feedback ? this.feedback.ta_da_incurred.toString() : ""
      ],
      comments: [
        this.feedback ? this.feedback.comments.toString() : "",
        Validators.required
      ],
      duration_rating: [
        this.feedback ? this.feedback.duration_rating.toString() : "",
        Validators.required
      ],
      content_rating: [
        this.feedback ? this.feedback.content_rating.toString() : "",
        Validators.required
      ],
      methodology_rating: [
        this.feedback ? this.feedback.methodology_rating.toString() : "",
        Validators.required
      ],
      admin_service_rating: [
        this.feedback ? this.feedback.admin_service_rating.toString() : "",
        Validators.required
      ],
      overall_utility_rating: [
        this.feedback ? this.feedback.overall_utility_rating.toString() : "",
        Validators.required
      ],
  ...
  submitFeedback() {
    ...
    if (this.feedback && this.feedback.id) {
      this.feedbackService
        ...
        .subscribe(
          (myFeedback: TrainingFeedback) => {
            console.log(myFeedback);
            this.isLoading = false;
            // find the index of old feedback before update
            const index = this.training.training_feedbacks.findIndex(
              data => data.id == this.feedback.id
            );
      ...
      this.feedbackService
        .addFeedback(this.training.id, this.feedbackForm.value)
        .subscribe(
          (myFeedback: TrainingFeedback) => {
            this.isLoading = false;
