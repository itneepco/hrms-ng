...
export class TrainingLabelService {
  ...
  getTrainingLabels() {
    return this.http
      .get<TrainingLabel[]>(this.training_label_url)
  ...
  addTrainingLabel(label: TrainingLabel) {
    return this.http
      .post(this.training_label_url, label)
      .pipe(catchError(err => this.handler.handleError(err)));
  }

  editTrainingLabel(id: number, label: TrainingLabel) {
    return this.http
      .put(`${this.training_label_url}/${id}`, label)
      .pipe(catchError(err => this.handler.handleError(err)));
  }

  deleteTrainingLabel(id: number) {
    return this.http
      .delete(`${this.training_label_url}/${id}`)
      .pipe(catchError(err => this.handler.handleError(err)));
  }
}
