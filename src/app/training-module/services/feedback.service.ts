...
export class FeedbackService {
  ...
  getFeedbacks(trainingId: number): Observable<TrainingFeedback[]> {
    return this.http
      .get<TrainingFeedback[]>(`${this.feedback_url}/${trainingId}/feedback`)
      .pipe(catchError(err => this.handler.handleError(err)));
  }

  addFeedback(trainingId: number, feedback: TrainingFeedback) {
    return this.http
      .post(`${this.feedback_url}/${trainingId}/feedback`, feedback)
      .pipe(catchError(err => this.handler.handleError(err)));
  }
  ...
  editFeedback(
    ...
    return this.http
      .put(
        `${this.feedback_url}/${trainingId}/feedback/${feedbackId}`,
        feedback
      )
      .pipe(catchError(err => this.handler.handleError(err)));
  }
  ...
  deleteFeedback(trainingId: number, feedbackId: number) {
    return this.http
      .delete(`${this.feedback_url}/${trainingId}/feedback/${feedbackId}`)
      .pipe(catchError(err => this.handler.handleError(err)));
  }
}
