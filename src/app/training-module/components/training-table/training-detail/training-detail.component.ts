...
export class TrainingDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.training.training_type === EXTERNAL_TRAINING) {
      this.trainingService
        .getTrainingInstitute(this.training.training_institute_id)
        .subscribe(data => {
          console.log(data);
  ...
  markParticipant(event, participant: Participant) {
    const temp = this.attendance.find(
      data => data.emp_code == participant.emp_code
    );

    if (event.checked) {
      if (temp) {
        return (temp.present = true);
      }

      this.attendance.push({ emp_code: participant.emp_code, present: true });
    } else {
      if (temp) {
        return (temp.present = false);
      }
  ...
  markAsPresent() {
    ...
    this.participantService
      .markPresent(this.training.id, this.attendance)
      .subscribe(() => {
        this.snackbar.open(
          "Successfully marked participants as present",
          "Dismiss",
          {
            duration: 1600
          }
  ...
  markTrainingCompleted() {
    this.trainingService
      .markTrainingCompleted(this.training.id)
      .subscribe(() => {
        this.training.status = TRAINING_COMPLETED;
        this.snackbar.open(
          "Successfully marked the training as completed",
          "Dismiss",
          {
            duration: 1600
          }
  ...
  calculateRating(feedback: TrainingFeedback) {
    const sum: number =
      feedback.admin_service_rating +
      feedback.content_rating +
      feedback.duration_rating +
      feedback.methodology_rating +
      feedback.overall_utility_rating;
    return sum / 5;
  }

  getEmployeeDetail(feedback: TrainingFeedback) {
    const emp = this.participants.data.find(
      participant => participant.emp_code == feedback.emp_code
    );
    if (!emp) {
      return "";
    }
  ...
  downloadOrder(id: number) {
    this.trainingService.downloadOrder(id).subscribe(data => {
      console.log(data);
