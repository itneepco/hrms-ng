...
export class LeaveTableComponent implements OnInit {
  ...
  getLeaveType(leaveApplication: LeaveApplication) {
    const el_type = leaveApplication.leaveDetails.find(leaveDetail => leaveDetail.leave_type == EL_CODE);
    if (el_type) { return "EL" }

    const hpl_type = leaveApplication.leaveDetails.find(leaveDetail => leaveDetail.leave_type == HPL_CODE);
    if (hpl_type) { return "ML/HPL" }

    const cl_rh_type = leaveApplication.leaveDetails
