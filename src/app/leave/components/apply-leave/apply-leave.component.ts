...
export class ApplyLeaveComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.ledgerService
      .getCurrYearBal(this.authService.currentUser.emp_code)
      .subscribe((register: LeaveRegister) => {
        this.leaveStatuses = register.status;
      });

    this.hierarchyService
      .getParents(this.authService.currentUser.emp_code)
    ...
    this.leaveForm = this.fb.group(
      ...
      },
      {
        validator: [
          ...
          EL_CODE
            ? this.checkElBalance.bind(this)
            : this.checkHPLBalance.bind(this)
        ]
      }
    );
  }
  ...
  applyLeave() {
    ...
    let leavApplication: LeaveAppForm = Object.assign(this.leaveForm.value, {
      ...
      emp_code: this.authService.currentUser.emp_code
    });
  ...
  get hpl_balance(): number {
    let leave = this.leaveStatuses.find(
