...
export class LeaveDetailComponent implements OnInit, OnDestroy {
  ...
  checkBalance(): boolean {
    ...
    if (this.leaveType.isCasualLeave(this.leaveApp.leaveDetails) ||
      ...
      const no_of_cl = this.leaveType.noOfCasualLeave(leaveDetails) + this.leaveType.noOfHalfDayCL(leaveDetails);
  ...
  get cl_balance() {
    const leave = this.leaveStatuses.find(status => status.leave_code == CL_CODE);
    if (!leave) { return 0 }
  ...
  get rh_balance() {
    const leave = this.leaveStatuses.find(status => status.leave_code == RH_CODE);
    if (!leave) { return 0 }
  ...
  get el_balance() {
    const leave = this.leaveStatuses.find(status => status.leave_code == EL_CODE);
    if (!leave) { return 0 }
  ...
  get hpl_balance() {
    const leave = this.leaveStatuses.find(status => status.leave_code == HPL_CODE);
    if (!leave) { return 0 }
